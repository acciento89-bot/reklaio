...
const deleteSchema = z.object({ password: z.string().min(10).max(128), confirmation: z.string().trim().min(1).max(32) });
function settingsError(message: string) { const url = publicUrl("/einstellungen"); url.searchParams.set("error", message); return NextResponse.redirect(url, 303); }
...
export async function POST(request: Request) {
  ...
  const parsed = deleteSchema.safeParse({ password: formData.get("password"), confirmation: formData.get("confirmation") });
  if (!parsed.success) return settingsError("Bitte gib dein aktuelles Passwort und das Wort LÖSCHEN ein.");
  if (parsed.data.confirmation.toLocaleUpperCase("de-DE") !== "LÖSCHEN") return settingsError("Zur Bestätigung muss exakt das Wort LÖSCHEN eingegeben werden.");
  ...
  try {
    ...
    const accountResult = await client.query<{
      ...
      `SELECT password_hash,stripe_subscription_id,subscription_status
       FROM app_users WHERE id=$1 FOR UPDATE`, [user.id]
    );
    ...
    const documentResult = await client.query<{ storage_key: string }>(
      `SELECT d.storage_key FROM case_documents d JOIN cases c ON c.id=d.case_id WHERE c.user_id=$1`, [user.id]
    );
