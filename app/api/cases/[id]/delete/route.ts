...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  if (confirmation !== "LÖSCHEN") {
    const url = publicUrl(`/faelle/${id}`);
    url.searchParams.set("error", "Bitte gib zum endgültigen Löschen das Wort LÖSCHEN ein.");
  ...
  try {
    ...
    const url = publicUrl(`/faelle/${id}`);
    url.searchParams.set("error", "Der Fall konnte gerade nicht gelöscht werden.");
    return NextResponse.redirect(url, 303);
  } finally {
    client.release();
  }
