  ...
  return (
    <main className="app-shell">
      ...
      <section className="app-content">
        ...
        <div className="dashboard-brand-banner"><img src="/reklaio-banner.svg" alt="Reklaio – Dein Fall. Deine Frist. Dein Überblick." /></div>
        ...
        <section className="panel dashboard-assistant-panel" id="aufmerksamkeit">
          <div className="panel-header">
            <div><span className="eyebrow">Fallassistent</span><h2>Was jetzt wichtig ist</h2></div>
            <span>{attentionCases.length} aktive Fälle bewertet</span>
          </div>
        ...
        <section className="dashboard-tools panel">
          <div className="dashboard-scope-tabs" aria-label="Fallansicht">
            {scopes.map((item) => <Link className={scope === item.value ? "active" : ""} href={dashboardHref(filterState, { scope: item.value })} key={item.value}>{item.label}</Link>)}
          </div>
          ...
          {allCases.length === 0 ? (
            ...
            <div className="case-list">
              {visibleCases.map((item) => {
                ...
                const assistant = assistantById.get(item.id)!;
                return (
                  <Link className={`case-row dashboard-case-row assistant-row-${assistant.priority}`} href={`/faelle/${item.id}`} key={item.id}>
