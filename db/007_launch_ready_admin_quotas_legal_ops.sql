...
ALTER TABLE app_users
  ADD COLUMN IF NOT EXISTS ai_document_limit_override INTEGER;

ALTER TABLE app_users
  ADD COLUMN IF NOT EXISTS ai_letter_limit_override INTEGER;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'app_users_ai_document_limit_override_check'
  ) THEN
    ALTER TABLE app_users
      ADD CONSTRAINT app_users_ai_document_limit_override_check
      CHECK (ai_document_limit_override IS NULL OR ai_document_limit_override >= -1);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'app_users_ai_letter_limit_override_check'
  ) THEN
    ALTER TABLE app_users
      ADD CONSTRAINT app_users_ai_letter_limit_override_check
      CHECK (ai_letter_limit_override IS NULL OR ai_letter_limit_override >= -1);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ai_usage_events_status_check'
  ) THEN
    ALTER TABLE ai_usage_events
...
CREATE TABLE IF NOT EXISTS billing_checkout_intents (
  ...
  immediate_start_requested_at TIMESTAMPTZ,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'billing_checkout_intents_status_check'
  ) THEN
    ALTER TABLE billing_checkout_intents
      ADD CONSTRAINT billing_checkout_intents_status_check
      CHECK (status IN ('created', 'redirected', 'completed', 'cancelled', 'failed'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_billing_checkout_intents_user_created
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'system_incidents_severity_check'
  ) THEN
    ALTER TABLE system_incidents
      ADD CONSTRAINT system_incidents_severity_check
      CHECK (severity IN ('info', 'warning', 'critical'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_system_incidents_open_created
  ON system_incidents(resolved_at, created_at DESC);
...
CREATE TABLE IF NOT EXISTS backup_runs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  request_id UUID REFERENCES backup_requests(id) ON DELETE SET NULL,
  database_file TEXT,
  uploads_file TEXT,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'backup_runs_status_check'
  ) THEN
