...
function actionHtml(action: EmailAction | undefined, accent: string, en = false) {
  if (!action) return "";
  ...
  <p style="margin:0 0 22px;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.55;color:#94a3b8;word-break:break-all;">${en ? "If the button does not work, open this link:" : "Falls der Button nicht funktioniert, öffne diesen Link:"}<br><a href="${url}" style="color:#5b5bd6;text-decoration:underline;">${url}</a></p>`;
}
...
export function renderReklaioEmail(options: ReklaioEmailOptions) {
  ...
  const homeUrl = escapeHtml(publicUrl(en ? "/en" : "/").toString());
  const contactUrl = escapeHtml(publicUrl(en ? "/en/kontakt" : "/kontakt").toString());
  const privacyUrl = escapeHtml(publicUrl(en ? "/en/datenschutz" : "/datenschutz").toString());
  const imprintUrl = escapeHtml(publicUrl(en ? "/en/impressum" : "/impressum").toString());
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:620px;border-collapse:separate;border-spacing:0;background:#ffffff;border-radius:22px;overflow:hidden;box-shadow:0 10px 35px rgba(15,23,42,.08);">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  ...
                  <td valign="middle" style="padding-left:12px;font-family:Arial,Helvetica,sans-serif;color:#ffffff;">
                    ...
                    <div style="margin-top:4px;font-size:11px;line-height:1.2;color:#b8c2e0;letter-spacing:.9px;text-transform:uppercase;">${en ? "Complaints under control" : "Reklamationen im Griff"}</div>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:34px 34px 20px;">
              ...
              ${actionHtml(options.action, colors.accent, en)}
          ...
          <tr>
            <td style="padding:20px 34px 28px;border-top:1px solid #e5e7eb;background:#fbfcfe;font-family:Arial,Helvetica,sans-serif;">
              <p style="margin:0 0 10px;font-size:12px;line-height:1.55;color:#94a3b8;">${en ? "This message was sent automatically by Reklaio." : "Diese Nachricht wurde automatisch von Reklaio gesendet."}</p>
              <p style="margin:0;font-size:12px;line-height:1.7;color:#64748b;">
                <a href="${homeUrl}" style="color:#5b5bd6;text-decoration:none;">reklaio.de</a>
                &nbsp;&middot;&nbsp;
                <a href="${contactUrl}" style="color:#5b5bd6;text-decoration:none;">${en ? "Contact" : "Kontakt"}</a>
                &nbsp;&middot;&nbsp;
                <a href="${privacyUrl}" style="color:#5b5bd6;text-decoration:none;">${en ? "Privacy" : "Datenschutz"}</a>
                &nbsp;&middot;&nbsp;
                <a href="${imprintUrl}" style="color:#5b5bd6;text-decoration:none;">${en ? "Legal notice" : "Impressum"}</a>
              </p>
            </td>
          </tr>
        </table>
