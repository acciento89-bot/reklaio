  ...
  if [ "$due" -eq 1 ]; then
    ...
    if [ -z "$error" ]; then
      if [ -d "$UPLOAD_DIR" ]; then
        ...
        mkdir -p "$UPLOAD_DIR"
        tar -C "$UPLOAD_DIR" -czf "$uploads_file" . || error="Empty upload backup failed"
      fi
    fi
    ...
    if [ -z "$error" ]; then
      ...
      if [ -n "$request_id" ]; then
        psql "$DATABASE_URL" -c "UPDATE backup_requests SET status='completed',completed_at=NOW() WHERE id='$request_id'" >/dev/null 2>&1 || true
      fi
      ...
      if [ -n "$request_id" ]; then
        psql "$DATABASE_URL" -c "UPDATE backup_requests SET status='failed',completed_at=NOW(),error_message='$safe_error' WHERE id='$request_id'" >/dev/null 2>&1 || true
      fi
